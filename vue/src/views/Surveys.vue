<template>
   <PageComponent>
     ...
     <!-- <div>
       <pre>{{surveys}}</pre>
     </div> check data surveys dah masuk dah keluar tak -->
     <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3">
        <div v-for="survey in surveys"
          ...
          <div class="flex justify-between items-center mt-3">
              ...
              <button v-if="survey.id" type="button" @click="deleteSurvey(survey)" class="h-8 w-8 flex items-center justify-center rounded-full border border-transparent text-sm text-red-500 focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
